...
livestream <- read_csv("../assignmentand data/Data_Livestream_General_processed.csv")
livestream
...
kol<- livestream %>% select(Name,Host_category) %>% distinct()
...
```{r}
#by categories
sales <-read_csv("../assignmentand data/Data_Sales_processed2.csv")
sales
sum(is.na(sales))
...
filter(Host_category=='K')%>%
  ggplot(aes(Sales_amount_total, Good_category_new)) + 
  geom_col(fill = 'red') + 
  facet_wrap(~Name, nrow=2)+
  scale_x_continuous(labels = scales::comma)+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1))

#beauty is the most popular category in celebraties. Other categories is pretty even 
sales2%>%
